    ...
    var Collector = {
        ...
        myDeviceHash : function () {
            try {
                ...
                console.error("aid_"+this.getCookie('aid')+"_myDeviceHash_error_\n"+err)
                if (window.TrackJS) {
                    TrackJS.addMetadata("aid", this.getCookie('aid'));
                    TrackJS.addMetadata("window.fingerprint.md5hash.myDeviceHash.error", err);
                }
                return this.stringToMD5(this.randomString(15));
            }
        }
